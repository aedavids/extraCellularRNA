...
pipelineStageName="best10CuratedDegree1"
...
if [ -z "${runSlurm}" ]; then
    ...
    if [[ $runSlurm != "SLURM" ]]; then
        ...
        slurm can not run python directly we would need to put this in a script sbatch my script
        # --job-name ${progName} create error
        # sbatch: error: This does not look like a batch script.  The first
        # sbatch: error: line must start with #! followed by the path to an interpreter.
        # sbatch: error: For instance: #!/bin/sh
        sbatch python -m models.randomForestHyperparmeterSearch \
                    --outDir ${outDir} \
                    --features Esophagus_Mucosa \
                    --pipelineStageName ${pipelineStageName} \
                    --elife 'Healthy donor' 'Esophagus Cancer' > $logFile 2>&1 &

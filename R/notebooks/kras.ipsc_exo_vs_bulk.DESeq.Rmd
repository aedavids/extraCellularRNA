...
hack
```{r}
hackFile <- '/home/kimlab/kras.ipsc/data/exo.data/gen1c.day.5.exo.input.data/ctrl.3/gencode.salmon.out/quant.sf'
hDF <- read.table(hackFile, header=TRUE)
hhDF <- head(hDF)
# newCols <- strsplit(hhDF$Name, "\\|")
#https://stackoverflow.com/questions/7069076/split-column-at-delimiter-in-data-frame
fooDF <- data.frame(do.call('rbind', strsplit(as.character(hhDF$Name),'|',fixed=TRUE)))
# https://github.com/rreggiar/welcome-to-the-kim-lab/wiki/Salmon-output-data-dictionary
colnames(fooDF) <- c("ENST", "ENSG", "OTTHUMG", "OTTHUMT", "HGNCT", "HGNCG", "refLength", "BioType") 
aedwipDF <- cbind(fooDF, hhDF)
```
...
readTx2GeneDF <- function() {
  ...
  #tx2geneDF <- gen.32.tx.2.geneDF[, c(1,5)] # 5 is HGNC with gene specific transcript id
  tx2geneDF <- gen.32.tx.2.geneDF[, c(1,6)] # 6 is HGNC required to stay at gene level
...
```{r}
colSums(upGeneCountsDF[1:50,])
colSums(downGeneCountsDF[1:50,])
```

save most important gene fragment counts
```{r}
csvName = paste0("DESeq.ctrl.", coefList[2], ".upRegulatedCounts.csv")
csvPath <- file.path( outputRoot, csvName)
write.csv(upGeneCountsDF, file=csvPath)

csvName = paste0("DESeq.ctrl.", coefList[2], ".downRegulatedCounts.csv")
csvPath <- file.path( outputRoot, csvName)

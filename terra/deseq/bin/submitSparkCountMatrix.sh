...
if [ "$#" -lt 3 ]; then
    echo "Usage: $0 clusterName gcp-project.config tcga-workspace-name labels"
    echo "labels are optional"
    echo "example tcga-workspace-name 'TCGA_READ_ControlledAccess_V1-0_DATA_edu_ucsc_kim_lab'"
...
mainPython="../python/bigDataDeseq/countMatrixCLI.py"
...
gcloud dataproc jobs submit pyspark ${mainPython} \
       ...
       --async \
       -- \
       --quantFilesCSV="gs://${BUCKET}/TCGA_META/${workspace}_quantFile.csv" \
       --outputDir="gs://${BUCKET}/spark.out/$0/${workspace}"

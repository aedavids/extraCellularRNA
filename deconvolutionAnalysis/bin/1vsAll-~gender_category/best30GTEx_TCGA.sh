...
# 11/15/23
# 
# runs analysis.BestSignatureGeneConfig
# topN biologically signifigant genes sorted by baseMean
#
# this is a "head" pipeline stage. ie. it does not depend on upstream stages
#
...
#
# How to run
# 1) cp ~/extraCellularRNA/deconvolutionAnalysis/bin/{best30GTEx_TCGA.sh,pipeline.sh} .;
# 2) best30GTEx_TCGA.sh cibersortSecurityToken ${USER}@ucsc.edu'
###### deprecated  2) setsid sh -c 'set -x; ./best25GTEx_TCGA.sh cibersortSecurityToken ${USER}@ucsc.edu' > best25GTEx_TCGA.sh.log 2>&1 &;
# 3) tail -f best25GTEx_TCGA.sh.log
#

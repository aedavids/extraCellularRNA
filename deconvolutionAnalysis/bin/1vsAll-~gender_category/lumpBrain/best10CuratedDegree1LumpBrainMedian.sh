...
#
# 5/29/24
# pipeline.sh calls pipeline.upstreamPipeline.py createSignatureMatrix().
# createSignatureMatrix() has an new boolean flag argument
# if true use median, else use mean to calculate expected count values for each gene in a category
# for backwards compatiblity the default value is False and we use an environmental variable
# to pass the flag
export USE_MEDIAN="useMedian"
